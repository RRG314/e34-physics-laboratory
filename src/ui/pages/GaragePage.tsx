...
export function GaragePage() {
  return (
    <div className="content-page garage-page">
      ...
      <section className="progression-rig"><p className="eyebrow">One supported car / increasing model depth</p><h2>The physics advances before the badge changes.</h2><div>{vehicleProgression.map((stage) => <article key={`${stage.year}-${stage.model}`} className={stage.year === 'High school' ? 'active' : ''}><span>{stage.year}</span><strong>{stage.model}</strong><h3>{stage.system}</h3><p>{stage.learning}</p><small>{stage.reason}</small></article>)}</div><p className="metaphor-note"><ShieldCheck size={15} /> Only the 1995 US manual 525i sedan is an active teaching vehicle. Other E34s remain research candidates until their exact data and redistributable visuals pass the admission gate.</p></section>
