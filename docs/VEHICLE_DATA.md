...
The downloaded multi-vehicle scene was reduced to its fully assembled 525i sedan. Its body and wheel were aligned to the laboratory axes, the source display floor and unused variants were removed, and the wheel was separated so four instances can rotate, be selected, and move outward in the first exploded-view lesson. The source white exterior and interior atlases are used. Roof and rear-pillar faces with incorrect source UV sampling are isolated under matching white paint, and the rear lamp lenses are reconstructed at the rear corners instead of leaving lamp pixels on the roof or door frame. The rendered scale is set from the laboratory's documented unloaded-wheel-radius assumption. The white homepage image comes from a different staged arrangement in the same source scene, so it is a visual reference rather than a frame captured from the interactive car.
